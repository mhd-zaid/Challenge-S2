...
const getData = async () => {
  try {
    ...
    const THIRTY_DAYS_AGO = new Date(Date.now() - THIRTY_DAYS).toISOString()
    const THIRTY_DAYS_INTERVAL = [THIRTY_DAYS_AGO, TODAY]
    const SIXTY_DAYS_AGO = new Date(Date.now() - THIRTY_DAYS * 2).toISOString()
    const SIXTY_DAYS_INTERVAL = [SIXTY_DAYS_AGO, THIRTY_DAYS_AGO]

    const promises = [
      axiosInstance.post('/stats/registrations', { dates: THIRTY_DAYS_INTERVAL }),
      axiosInstance.post('/stats/registrations', { dates: SIXTY_DAYS_INTERVAL })
    ]

    Promise.all(promises).then((responses) => {
      state.newUsersLast30Days = responses[0].data
      state.newUsersAugmentation = responses[0].data - responses[1].data
    })
  } catch (error) {
    console.log(error)
  }
      ...
      <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        ...
        >
          <dt>
            ...
            <p class="ml-16 truncate text-sm font-medium text-gray-500">Nouvelles inscriptions</p>
          </dt>
          <dd class="ml-16 flex items-baseline pb-6 sm:pb-7">
            <p class="text-2xl font-semibold text-gray-900">{{ state.newUsersLast30Days }}</p>
